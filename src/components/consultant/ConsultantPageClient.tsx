...
export function ConsultantPageClient() {
  ...
  const auctionsQuery = useQuery({
    queryKey: ["consultant-auctions"],
    queryFn: () => Promise.all([fetchAuctionCount(), fetchMyAuctions()]),
  });
  const items = auctionsQuery.data?.[1] ?? [];
  const total = auctionsQuery.data?.[0]?.total ?? 0;
  const pendingCount = auctionsQuery.data?.[0]?.pending ?? 0;
  const loading = auctionsQuery.isPending;
  const reloadData = () => queryClient.invalidateQueries({ queryKey: ["consultant-auctions"] });
