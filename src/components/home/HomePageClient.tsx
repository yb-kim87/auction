...
export function HomePageClient() {
  ...
  const strategyLabelOptionsQuery = useQuery({
    queryKey: ["strategy-label-options"],
    queryFn: fetchStrategyLabelOptions,
    staleTime: 5 * 60 * 1000,
  });
  const strategyLabelOptions = useMemo(
    () => (strategyLabelOptionsQuery.data ?? []).map((item) => item.label),
    [strategyLabelOptionsQuery.data],
  );
  ...
  // apiFilters를 쿼리키에 그대로 쓰면 필터/검색어가 바뀔 때마다 새 쿼리로
  // 취급되어 자동으로 1페이지부터 다시 불러온다 — 기존 loadRecommendations
  // useEffect가 하던 일을 React Query가 대신한다.
  ...
  const recommendationsQuery = useInfiniteQuery({
    queryKey: ["recommendations", currentBudget, apiFilters],
    queryFn: ({ pageParam }) =>
      fetchRecommendations(currentBudget, { limit: PAGE_SIZE, offset: pageParam }, apiFilters),
    initialPageParam: 0,
    getNextPageParam: (lastPage, allPages) =>
      lastPage.hasMore ? allPages.reduce((sum, page) => sum + page.items.length, 0) : undefined,
    // 필터가 바뀌는 순간 목록이 빈 화면으로 깜빡이지 않도록 이전 결과를 유지한 채 새로 불러온다.
  ...
  const loanInfoByItemId = useMemo(() => {
    const merged: Record<string, LoanInfo> = {};
    for (const page of recommendationsQuery.data?.pages ?? []) {
      Object.assign(merged, page.loanInfoByItemId);
    }
    return merged;
  }, [recommendationsQuery.data]);
  ...
  const loadError = recommendationsQuery.isError
  ...
  function loadMoreRecommendations() {
    if (recommendationsQuery.isFetchingNextPage || !recommendationsQuery.hasNextPage) return;
  ...
  const toggleFavoriteMutation = useMutation({
    ...
    onMutate: async ({ auctionId, next, category, memo }) => {
      ...
      // 클릭 직후 하트와 상단 건수가 반응하도록 먼저 캐시를 낙관적으로 갱신한다.
      // 네트워크 요청이 실패하면 onError에서 이 물건만 원래 상태로 되돌린다.
    ...
    onSettled: () => setFavoriteBusyId(null),
  });
  ...
  return (
    <div className="min-h-screen bg-background" style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <AuctionDetailModal
        ...
        onAiAnalysisUsed={() =>
          patchProfile({ aiAnalysisUsed: (profile?.aiAnalysisUsed ?? 0) + 1 })
        }
      />
