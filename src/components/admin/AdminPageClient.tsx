...
import { AuctionFormModal } from "@/app/admin/AuctionFormModal";
...
import { CrawlerWorkPanel } from "@/app/admin/CrawlerWorkPanel";
import { KnowledgePanel } from "@/app/admin/KnowledgePanel";
import { RightsRulesTab } from "@/app/admin/RightsRulesTab";
import { LoanPolicyTab } from "@/app/admin/LoanPolicyTab";
import { TagRulesTab } from "@/app/admin/TagRulesTab";
import { StrategyTagsTab } from "@/app/admin/StrategyTagsTab";
import { AiPlatformPanel } from "@/app/admin/AiPlatformPanel";
import { KakaoNotifyPanel } from "@/app/admin/KakaoNotifyPanel";
import { CoupangSourcingTab } from "@/app/admin/CoupangSourcingTab";
import { SecurityLogTab } from "@/app/admin/SecurityLogTab";
import { SwimApplyTab } from "@/app/admin/SwimApplyTab";
import { ResaleMatchTab } from "@/app/admin/ResaleMatchTab";
import { RealtorCollectTab } from "@/app/admin/RealtorCollectTab";
import { AssignmentReviewTab } from "@/app/admin/AssignmentReviewTab";
...
export function AdminPageClient() {
  ...
  const countsQuery = useQuery({ queryKey: ["admin-auction-counts"], queryFn: fetchAuctionCount });
  const total = countsQuery.data?.total ?? 0;
  const pendingCount = countsQuery.data?.pending ?? 0;

  const itemsQuery = useQuery({
    queryKey: ["admin-auctions", listPage, listPageSize, appliedSearch],
    queryFn: () => fetchAdminAuctions({ page: listPage, pageSize: listPageSize, search: appliedSearch }),
  });
  const items = itemsQuery.data?.items ?? [];
  const listTotal = itemsQuery.data?.total ?? 0;
  const listTotalPages = itemsQuery.data?.totalPages ?? 1;
  const loadingItems = itemsQuery.isPending;
  // 삭제 등으로 마지막 페이지가 사라지면 서버가 알려주는 totalPages로 자동 보정한다.
  useEffect(() => {
    if (itemsQuery.data && listPage > itemsQuery.data.totalPages) {
      setListPage(itemsQuery.data.totalPages);
    }
  }, [itemsQuery.data, listPage]);

  const pendingQuery = useQuery({ queryKey: ["admin-pending-auctions"], queryFn: fetchPendingAuctions });
  ...
  const reloadData = useCallback(
    () =>
      queryClient.invalidateQueries({
        predicate: (query) =>
          ["admin-auction-counts", "admin-auctions", "admin-pending-auctions", "admin-users"].includes(
  ...
  // 목록이 새로 불러와지면(페이지 이동, 검색, 새로고침 등) 이전 화면 기준으로
  // 골라둔 체크박스 선택은 더 이상 유효하지 않으므로 비운다.
  useEffect(() => {
        ...
        nav={
          <>
            ...
            <div className={HEADER_NAV_TRAILING}>
              ...
              <Link href="/courses/my" className={HEADER_BTN}>
                강의실
              </Link>
              <AccountNavLink />
