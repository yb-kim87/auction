...
export default function BidCalendarPage() {
  ...
  useEffect(() => {
    Promise.allSettled([fetchFavorites(), fetchMyBidPlans(), fetchAssignments()]).then(async ([f, p, a]) => {
      const fav = f.status === "fulfilled" ? f.value : [];
      const plan = p.status === "fulfilled" ? p.value : [];
      const assignment = a.status === "fulfilled" ? a.value : [];
      setFavorites(fav); setPlans(plan); setAssignments(assignment);
      const ids = Array.from(new Set([...fav.map(x => x.auctionId), ...plan.map(x => x.auctionId), ...assignment.map(x => x.auctionId)]));
